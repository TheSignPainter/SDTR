...
class Trainer(nn.Module):
    def __init__(self, model, loss_function, experiment_name=None, warm_start=False, 
                 Optimizer=torch.optim.Adam, params_to_opt=None, optimizer_params={}, verbose=False, 
    ...
    def train_on_batch(self, *batch, device):
        ...
        if type(result) == tuple:
            assert len(result)==3
            result, reg_loss, l1_loss = result
            metrics['reg_loss'] = reg_loss.item()
            metrics['l1_loss'] = l1_loss.item()
            other_loss = reg_loss + l1_loss    
        loss = self.loss_function(result, y_batch).mean()
    ...
    def evaluate_entropy(self):
        return self.model.eval_entropy()